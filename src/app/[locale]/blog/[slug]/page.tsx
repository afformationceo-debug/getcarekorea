          ...
          >
            <Card className="overflow-hidden border-0 shadow-xl">
              <CardContent className="p-8 lg:p-12">
                ...
                {/* In-Content CTA - Right after article */}
                {(() => {
                  const ctaConfig = getMessengerCTA(locale, post.authorPersona);
                  const isWhatsApp = ctaConfig.messenger === 'whatsapp';
                  const isLine = ctaConfig.messenger === 'line';
                  const isKakao = ctaConfig.messenger === 'kakao';

                  return (
                    <div className={`mt-12 p-6 lg:p-8 rounded-2xl border-2 relative overflow-hidden ${
                      isWhatsApp ? 'border-green-400 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/40 dark:to-emerald-950/40 dark:border-green-600' :
                      isLine ? 'border-green-400 bg-gradient-to-br from-green-50 to-teal-50 dark:from-green-950/40 dark:to-teal-950/40 dark:border-green-600' :
                      isKakao ? 'border-yellow-400 bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-950/40 dark:to-amber-950/40 dark:border-yellow-600' :
                      'border-violet-400 bg-gradient-to-br from-violet-50 to-purple-50 dark:from-violet-950/40 dark:to-purple-950/40 dark:border-violet-600'
                    }`}>
                      {/* Decorative elements */}
                      <div className={`absolute -top-12 -right-12 w-32 h-32 rounded-full blur-3xl opacity-30 ${
                        isWhatsApp || isLine ? 'bg-green-400' : isKakao ? 'bg-yellow-400' : 'bg-violet-400'
                      }`} />
                      <div className={`absolute -bottom-8 -left-8 w-24 h-24 rounded-full blur-2xl opacity-20 ${
                        isWhatsApp || isLine ? 'bg-emerald-400' : isKakao ? 'bg-amber-400' : 'bg-purple-400'
                      }`} />
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="flex items-center gap-2 mb-4">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${
                            isWhatsApp || isLine ? 'bg-green-500 text-white' :
                            isKakao ? 'bg-yellow-500 text-black' :
                            'bg-violet-500 text-white'
                          }`}>
                            <Sparkles className="h-3 w-3" />
                            {locale === 'ko' ? '무료 상담' : locale === 'ja' ? '無料相談' : 'FREE Consultation'}
                          </span>
                          <span className="text-xs text-muted-foreground">
                            {locale === 'ko' ? '24시간 내 답변' : locale === 'ja' ? '24時間以内に返信' : 'Reply within 24h'}
                          </span>
                        </div>

                        {/* Main Content */}
                        <h4 className={`text-xl lg:text-2xl font-bold mb-2 ${
                          isWhatsApp || isLine ? 'text-green-800 dark:text-green-200' :
                          isKakao ? 'text-yellow-800 dark:text-yellow-200' :
                          'text-violet-800 dark:text-violet-200'
                        }`}>
                          {locale === 'ko' ? '🤔 아직 궁금한 점이 있으신가요?' :
                           locale === 'ja' ? '🤔 まだ質問がありますか？' :
                           locale === 'zh-TW' || locale === 'zh-CN' ? '🤔 還有問題嗎？' :
                           '🤔 Still Have Questions?'}
                        </h4>
                        <p className="text-muted-foreground mb-6 max-w-xl">
                          {locale === 'ko' ? '저희 전문 코디네이터가 1:1로 상담해드립니다. 비용, 일정, 병원 추천까지 무엇이든 물어보세요!' :
                           locale === 'ja' ? '専門コーディネーターが1対1でご相談に応じます。費用、スケジュール、病院のおすすめまで、何でもお聞きください！' :
                           'Our expert coordinators provide personalized 1-on-1 consultations. Ask us anything about costs, schedules, or hospital recommendations!'}
                        </p>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className={`inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-semibold text-base transition-all transform hover:scale-[1.02] active:scale-[0.98] shadow-lg ${
                              isWhatsApp ? 'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white shadow-green-500/30' :
                              isLine ? 'bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white shadow-green-500/30' :
                              isKakao ? 'bg-gradient-to-r from-yellow-400 to-amber-500 hover:from-yellow-500 hover:to-amber-600 text-black shadow-yellow-500/30' :
                          ...
                          >
                            ...
                            {!isWhatsApp && !isLine && !isKakao && <MessageCircle className="h-5 w-5" />}
                            {ctaConfig.label}
                          </a>
                          <a
                            href={`/${locale}/contact`}
                            className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-semibold text-base border-2 transition-all hover:bg-muted/50 border-current text-muted-foreground"
                          >
                            <Phone className="h-4 w-4" />
                            {locale === 'ko' ? '이메일 문의' : locale === 'ja' ? 'メールで問い合わせ' : 'Email Us'}
                          </a>
                        </div>

                        {/* Trust indicators */}
                        <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-current/10">
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            <CheckCircle className="h-4 w-4 text-green-500" />
                            {locale === 'ko' ? '100% 무료 상담' : '100% Free'}
                          </div>
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            <CheckCircle className="h-4 w-4 text-green-500" />
                            {locale === 'ko' ? '한국어 통역 지원' : 'Interpreter Available'}
                          </div>
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            <CheckCircle className="h-4 w-4 text-green-500" />
                            {locale === 'ko' ? '8년+ 경력' : '8+ Years Experience'}
                          </div>
                        </div>
                      </div>
                    </div>
